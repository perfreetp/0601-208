...
function PuzzleTab() {
  const { puzzlePieces } = useGameStore();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="glass-panel px-4 py-2">
          ...
          <span className="text-white/60"> / {puzzlePieces.length} 块</span>
        </div>
      </div>
      ...
      <div className="glass-panel p-6">
        <div className="grid grid-cols-3 gap-2 aspect-square max-w-md mx-auto">
          {Array.from({ length: 9 }).map((_, idx) => {
            const piece = puzzlePieces.find((_, i) => i === idx);
      ...
      <div className="glass-panel p-4">
        <h4 className="text-white font-semibold mb-3">未放置的拼图块</h4>
        <div className="flex flex-wrap gap-2">
          {puzzlePieces.filter(p => !p.isPlaced).map(piece => (
            <div
              key={piece.id}
              className="w-14 h-14 rounded-lg bg-ocean-mid border border-white/20 flex items-center justify-center text-2xl cursor-move hover:border-neon-cyan/50 hover:bg-ocean-light transition-all"
            >
              🧩
            </div>
          ))}
...
function FindItemTab() {
  const { hiddenItems } = useGameStore();
            ...
            <div
              className={cn(
                ...
                item.found
                  ...
                  : 'bg-white/5 border-2 border-white/20 opacity-40 hover:opacity-100 hover:border-aurora-green/50 cursor-pointer'
              )}
            >
...
function VoteTab() {
  const { voteTopics, castVote } = useGameStore();
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        hasVoted ? 'cursor-default' : 'hover:border-gold-yellow/50 hover:bg-white/10',
...
function QuizTab() {
  const { quizQuestions, currentQuizIndex, answerQuiz, teamScore } = useGameStore();
  ...
  const handleAnswer = (idx: number) => {
    ...
    setShowResult(true);
    answerQuiz(currentQuizIndex, idx);
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <div className="glass-panel px-4 py-2">
            <span className="text-coral-orange font-orbitron font-bold">⏱ {timer}s</span>
          </div>
          <div className="glass-panel px-4 py-2">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isCorrect && 'border-aurora-green bg-aurora-green/20 text-white',
                    isWrong && 'border-coral-orange bg-coral-orange/20 text-white',
                    !showResult && isSelected && 'border-neon-cyan bg-neon-cyan/10',
                    !showResult && !isSelected && 'border-white/10 bg-white/5 hover:border-white/30 hover:bg-white/10',
                    showResult && !isCorrect && !isWrong && 'border-white/10 bg-white/5 text-white/50'
                  )}
                >
                  <div className="flex items-center gap-3">
                    <span className={cn(
                      'w-8 h-8 rounded-full flex items-center justify-center font-bold',
                      isCorrect ? 'bg-aurora-green text-deep-ocean' :
                      isWrong ? 'bg-coral-orange text-white' :
          ...
          {showResult && (
            <div className="mt-6 text-center animate-fade-in">
              <p className={cn(
                ...
                selectedAnswer === currentQuestion.correctAnswer ? 'text-aurora-green' : 'text-coral-orange'
              )}>
              ...
              <button
                onClick={() => {
                  ...
                  setTimer(quizQuestions[Math.min(currentQuizIndex + 1, quizQuestions.length - 1)]?.timeLimit || 20);
                }}
